  ...
  try {
    ...
    if (process.env.RESEND_API_KEY) {
      await resend.emails.send({
        from: process.env.RESEND_FROM_EMAIL ?? 'Process <onboarding@resend.dev>',
        to: cleanEmail,
        subject: `You've been invited to ${org?.name ?? 'Process'}`,
        html: `
          <div style="font-family:Inter,sans-serif;max-width:600px;margin:0 auto;background:#FBFBF9">
            <div style="background:#E05236;padding:32px 40px;border-radius:16px 16px 0 0">
              <h1 style="color:white;font-size:24px;margin:0;font-weight:700">Process</h1>
            ...
            <div style="padding:40px;background:white;border-radius:0 0 16px 16px;border:1px solid #E5E3DF;border-top:none">
              ...
              <p style="color:#57534E;font-size:15px;line-height:1.6;margin:0 0 24px">
                <strong>${sender ?? 'An admin'}</strong> has invited you to join <strong>${org?.name ?? 'the workspace'}</strong> on Process — the design review and project management platform.
              </p>
